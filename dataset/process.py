...
def match_marking_points(ground, predict, params):
    ...
    # 1 for 30 degrees and 3 for 10 degrees
    # ratio = 1
    # ratio = 2
    # ratio = 3
    ratio = 6
    ...
    effect_flag = entry_startpoint < params.squared_distance_thresh \
                  and entry_endpoint < params.squared_distance_thresh*5 \
...
def subtract(a, b):
    return 1e-8 if a - b == 0 else a - b

def rectify_out_of_boundary(xd, yd, xa, ya, w, h):
    x, y = xd, yd
    if xd > w - 1:
      y = ya + (yd - ya) * ((w - 1 - xa) / subtract(xd, xa))    # x=w-1
      x = w - 1
      if y > h - 1:
        x = xa + (h - 1 - ya) * ((xd - xa) / subtract(yd, ya))  # y=h-1
        y = h - 1
    if y > h - 1:
      x = xa + (h - 1 - ya) * ((xd - xa) / subtract(yd, ya))    # y=h-1
      y = h - 1
...
def expand_to_boundary(xd, yd, xa, ya, w, h):
    x, y = xd, yd
    if xd < w - 1:
      x = w - 1
      y = ya + (yd - ya) * ((w - 1 - xa) / subtract(xd, xa))    # x=w-1
